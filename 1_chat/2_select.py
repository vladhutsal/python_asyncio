# TODO:
# There is a bunch of functions that could possibly block the execution flow
# Find and mark them
...
def echo_message(client_sock: socket.socket):
    ...
    if recieved_data:
        echo_msg = get_echo_response(recieved_data, client_sock)
        client_sock.send(echo_msg)
    else:
...
def run(serv_sock: socket.socket):
    ...
    while True:
        ...
        ready_to_read_list, _, _ = select(client_sockets, [], [])
...
def get_echo_response(data: bytes, client_sock: socket.socket) -> bytes:
    str_data = data.decode('UTF-8')
    print(f'message from {client_sock.getpeername()}: {str_data}')
